 ...
 */
function render_header(array $breadcrumbs, bool $active): string
{
...
<header class="site-header">
    <div class="utility-bar">
        <div class="container utility-bar__inner">
            ...
            <a class="icon-btn lang-switch" href="#" aria-label="Switch to English">EN</a>
...
foreach ($departments as $dept) {
    ...
    $cardsHtml .= <<<HTML
                <article class="dept-card" data-searchable data-category="{$dept['category']}" data-search="{$searchBlob}">
                    <span class="dept-card__icon" aria-hidden="true">{$categoryIcon}</span>
                    <span class="dept-card__category">{$categoryLabel}</span>
                    <h2 class="dept-card__title">
                        <a href="{$href}">{$dept['title']}</a>
                    </h2>
...
<main id="main">
    ...
    <section class="departments">
        <div class="container">
            <div class="departments__grid" data-dept-grid>
{$cardsHtml}
            </div>
    ...
    <section class="newsletter">
        <div class="container newsletter__inner">
            ...
            <form class="newsletter__form">
                ...
                <input class="newsletter__input" type="email" id="newsletterEmail" placeholder="عنوان بريدك الإلكتروني" required>
...
$byOrder = [];
foreach ($departments as $dept) {
    $byOrder[$dept['order']] = $dept;
}
$orders = array_keys($byOrder);
sort($orders);
$total = count($orders);
...
foreach ($departments as $index => $dept) {
    ...
    foreach ($categories as $catKey => $cat) {
        ...
        $openAttr = $dept['category'] === $catKey ? ' open' : '';
        $catIcon = $categoryIcons[$catKey];
        $sideHtml .= <<<HTML
                <details class="detail-side__group"{$openAttr}>
                    <summary>
...
HTML;
    ...
    $respItemsHtml = '';
    foreach ($dept['tasks'] as $task) {
        $respItemsHtml .= '<li class="resp-item">' . e($task) . '</li>';
    }

    $isLong = $totalTasks > 8;
    $wrapClass = $isLong ? ' is-collapsed' : '';
    $isLongAttr = $isLong ? ' data-collapsible' : '';
    $showMoreHtml = '';
    if ($isLong) {
        $showMoreHtml = <<<HTML
